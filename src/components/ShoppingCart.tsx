      ...
      <div className='flex flex-col justify-center items-center'>
        ...
        <div className='ml-10 mt-20 lg:mt-32 mr-10 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 md:gap-12 justify-center items-center'>
          {shoppingcart.map((val) => {
            const quantity = getItemQuantity(val.id); // Use val.id here

            return (
              <div key={val.id} className='mb-20'>
                ...
                <div className='cardItems cursor-pointer mt-6 pl-4 md:mr-16'>
                  ...
                  <div className='flex flex-row items-center justify-center gap-4'>
                    {quantity === 0 ? ( 
                      <button className='btnn btnText md:w-full md:mt-2 mt-1 mb-1 md:mb-0 md:ml-4' onClick={() => increaseCartQuantity(val.id)}>
                        + Add to cart
                      </button>
                    ) : (
                      <div className='flex flex-col items-center justify-center gap-[0.5rem]'>
                        <div className='flex flex-row items-center text-center justify-center gap-1 mt-1'>
                          <button className='p-3 bg-blue-400 rounded-md text-white' onClick={() => decreaseCartQuantity(val.id)}>-</button>
                          <div>
                            <span className='text-[19px]'>{quantity} {" "}</span> 
                            in cart
                          </div>
                          <button className='p-3 bg-blue-400 rounded-md text-white' onClick={() => increaseCartQuantity(val.id)}>+</button>
                        </div>
                        <button className='flex items-center justify-center gap-[0.5rem] bg-red-600 p-2 rounded-lg text-white' onClick={() => removeFromCart(val.id)}>
                          Remove
                        </button>
                      </div>
                    )}
